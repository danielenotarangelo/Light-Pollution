...
import LGIHealthChart from './LGIHealthChart.jsx';
...
export default function LGIHealthPanel({ lookup, country, year, healthMetric = 'd', dark, open, onClose, inStack = false, bgColor, compact = false }) {
  ...
  const corrData = series
    .filter(d => d.lgi != null && Math.abs(d.lgi) > 1e-8 && d[healthMetric] != null)
    .map(d => ({ x: d.lgi * 100, y: d[healthMetric] }));
  ...
  return (
    ...
    >
      <div className="fp-head">
        <div>
          <div className="fp-label">Luminosity growth &amp; mental health over time</div>
          <h2>LGI & {metricShort}</h2>
